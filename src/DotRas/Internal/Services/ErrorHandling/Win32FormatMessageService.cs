    ...
    internal class Win32FormatMessageService : IWin32FormatMessage {
        ...
        public string FormatMessage(int errorCode) {
            ...
            try {
                ...
                return ShouldThrowErrorFromLength(length) ? throw new Win32Exception() : marshaller.PtrToUnicodeString(lpBuffer, length);
            }
            finally {
        ...
        private static bool ShouldThrowErrorFromLength(int length) => length == 0;
    }
}
