...
namespace DotRas.Internal.Services.Connections {
    internal class RasGetConnectStatusService : IRasGetConnectStatus {
        ...
        public RasConnectionStatus GetConnectionStatus(IRasConnection connection) {
            ...
            return new RasConnectionStatus(rasConnStatus.rasconnstate, GetErrorCode(rasConnStatus.dwError), CreateDevice(rasConnStatus), rasConnStatus.szPhoneNumber, CreateLocalIpAddress(rasConnStatus), CreateRemoteIpAddress(rasConnStatus), rasConnStatus.rasconnsubstate);
        }

        private int? GetErrorCode(int dwError) {
            return dwError == SUCCESS ? null : (int?)dwError;
        }
        ...
        private RASCONNSTATUS GetConnectionStatusByHandle(IntPtr handle) {
            ...
            return ret != SUCCESS ? throw exceptionPolicy.Create(ret) : rasConnStatus;
        }

        private RasDevice CreateDevice(RASCONNSTATUS rasConnStatus) => deviceTypeFactory.Create(rasConnStatus.szDeviceName, rasConnStatus.szDeviceType);

        private IPAddress CreateLocalIpAddress(RASCONNSTATUS rasConnStatus) => ipAddressConverter.ConvertFromEndpoint(rasConnStatus.localEndpoint);

        private IPAddress CreateRemoteIpAddress(RASCONNSTATUS rasConnStatus) => ipAddressConverter.ConvertFromEndpoint(rasConnStatus.remoteEndpoint);
    }
}
