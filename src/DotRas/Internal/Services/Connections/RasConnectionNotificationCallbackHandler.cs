...
namespace DotRas.Internal.Services.Connections {
    internal class RasConnectionNotificationCallbackHandler : IRasConnectionNotificationCallbackHandler {
        ...
        private IList<RasConnection> FindConnectionChanges(RasConnection[] current) {
            return current.Length > previousState.Length ? FindChanges(current, previousState) : FindChanges(previousState, current);
        }

        private static IList<RasConnection> FindChanges(IEnumerable<RasConnection> collectionA, IEnumerable<RasConnection> collectionB) => collectionA.Where(o => !collectionB.Contains(o)).ToArray();
        ...
        private void ExecuteCallbackForChanges(Action<RasConnectionEventArgs> callback, IEnumerable<RasConnection> changes) {
            foreach (var connection in changes) {
                callback.Invoke(new RasConnectionEventArgs(new RasConnectionInformation(connection.Handle, connection.EntryName, connection.PhoneBookPath, connection.EntryId, connection.CorrelationId)));
            }
        }
    }
}
