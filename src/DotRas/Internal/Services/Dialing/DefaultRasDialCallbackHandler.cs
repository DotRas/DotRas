...
namespace DotRas.Internal.Services.Dialing {
    internal class DefaultRasDialCallbackHandler : DisposableObject, IRasDialCallbackHandler {
        ...
        private void HangUpConnection() => rasHangUp.UnsafeHangUp(handle.Value, false, CancellationToken.None);

        private void ExecuteStateChangedCallback(RasConnectionState connectionState) => onStateChangedCallback(new StateChangedEventArgs(connectionState));
        ...
        private static bool HasConnectionCompleted(RasConnectionState connectionState) => connectionState == RasConnectionState.Connected;

        private void RunPostCompleted() => onCompletedCallback();
        ...
        protected virtual RasConnection CreateConnection(IntPtr handle) => rasEnumConnections.EnumerateConnections().SingleOrDefault(o => o.Handle == handle);
        ...
        private void FlagRequestAsCompleted() => Completed = true;

        private void FlagRequestAsEncounteredErrors() => HasEncounteredErrors = true;
        ...
        private void WaitForHandleToBeTransferred() => handle.WaitForValue(cancellationToken);
        ...
        private void GuardRequestShouldNotBeCancelled() => cancellationToken.ThrowIfCancellationRequested();
    }
}
