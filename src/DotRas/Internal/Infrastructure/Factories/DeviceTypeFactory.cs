...
namespace DotRas.Internal.Infrastructure.Factories {
    internal class DeviceTypeFactory : IDeviceTypeFactory {
        ...
        public RasDevice Create(string name, string deviceType) {
            ...
            return factory == null ? CreateUnknownDevice(name, deviceType) : factory.Create(name);
        }

        private IDeviceFactory GetFactoryFromDeviceType(string deviceType) {
            return LookupTable.TryGetValue(deviceType, out var type) ? serviceLocator.GetService(type) as IDeviceFactory : null;
        }

        private static RasDevice CreateUnknownDevice(string name, string deviceType) => new Unknown(name, deviceType);
    }
}
