...
namespace DotRas.Internal.Policies {
    internal class DefaultExceptionPolicy : IExceptionPolicy {
        ...
        private bool IsRasErrorCode(int error) => error >= RASBASE && error <= RASBASEEND;

        private bool IsIPSecErrorCode(int error) => error >= IPSECBASE && error <= IPSECBASEEND;
        ...
        private Exception CreateIPSecException(int error) => new IPSecException(error);
    }
}
